    ...
    async def get_gas_usage_data(self) -> dict[str, Any]:
        ...
        try:
            ...
            for gas_service in gas_services:
                ...
                # Mercury returns gas as parallel (estimate, actual) pairs —
                # one entry per group per period, with one zero per pair.
                # pymercury 1.1.3+ exposes consumption_periods (pre-collapsed);
                # older versions only have the flat 20-entry daily_usage.
                # Collapse locally when the newer property is unavailable so
                # the importer never sees the zero-pair partner overwriting
                # a real estimate.
                consumption_periods = getattr(
                    gas_monthly, "consumption_periods", None
                )
                if consumption_periods is not None:
                    monthly_history = list(consumption_periods)
                else:
                    raw_history = list(
                        getattr(gas_monthly, "daily_usage", []) or []
                    )
